...
"""
pick_inN2 is the picking scheme which picks the Newton point whenever possible. 
...
"""
pick_inN is the picking scheme which picks the Newton point whenever possible but 
         uses pick_inInf whenever b=∞

Pick_inN2 is more efficient.
...
"""
pick_inN3 is the picking scheme which picks the Newton point whenever possible and then eventually
    ...
    global function pick_inN3(a, b, ϕ, ϕa, ϕb, dϕa, dϕb, forward)
        ...
        if forward
            ...
            if (dN < 0) & (dN > -γ*L)
                ...
                d4 = -(dp(dN3) / d2p(dN3))
                dN4 = dN3 + d4
                # testing shows that dn4 is most efficient, dN5 yields no improvement in counters,
                # but some overhead. TestSet: Cutest problems, n=1-1500, atol=rtol = 1e-8
                #d5 = -(dp(dN4) / d2p(dN4))
                #dN5 = dN4 + d5
                if (dN4 > 0) & (dN4 < γ*L)
                    t2 = b + dN4
